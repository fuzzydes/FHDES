...
class FHDES(BaseDES):
    ...
    def __init__(self, pool_classifiers=None,
        ...
        super(FHDES, self).__init__(pool_classifiers=pool_classifiers,
                                    with_IH=with_IH,
                                    safe_k=safe_k,
                                    IH_rate=IH_rate,
    ...
    def fit(self, X, y):
        ...
        if self.mis_sample_based == True:
            for classifier_index in range(self.n_classifiers_):
                ...
                Contraction_indexes = self.DSEL_processed_[:, classifier_index]
                self.setup_hyperboxs(MissSet_indexes, classifier_index, Contraction_indexes)
        else:
            for classifier_index in range(self.n_classifiers_):
                WellSet_indexes = self.DSEL_processed_[:, classifier_index]
                Contraction_indexes = ~self.DSEL_processed_[:, classifier_index]
                self.setup_hyperboxs(WellSet_indexes, classifier_index,Contraction_indexes)
    ...
    def setup_hyperboxs(self, samples_ind, classifier,Contraction_ind):
        ...
        for X in selected_samples:
            ...
            for box in boxes:
                ...
                if dist < nDist:
                    nearest_box = box
                    nDist = dist
            if (not nearest_box.will_exceed(X, contraction_samples) or  not self.doContraction) and (nearest_box.is_expandable(X) or not self.thetaCheck):
