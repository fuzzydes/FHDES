...
def initialize_ds(pool_classifiers, uncalibratedpool, X_DSEL, y_DSEL, k=7):
    ...
    fhW_cN_tY = FHDES(pool_classifiers, k=k, theta=theta, mu=NO_Hyperbox_Thereshold, mis_sample_based=False, doContraction=False, thetaCheck=True)
    fhM_cN_tY = FHDES(pool_classifiers, k=k, theta=theta, mu=NO_Hyperbox_Thereshold, mis_sample_based=True,  doContraction=False, thetaCheck=True)

    fhW_cY_tN = FHDES(pool_classifiers, k=k, theta=theta, mu=NO_Hyperbox_Thereshold, mis_sample_based=False, doContraction=True, thetaCheck=False)
    fhM_cY_tN = FHDES(pool_classifiers, k=k, theta=theta, mu=NO_Hyperbox_Thereshold, mis_sample_based=True,  doContraction=True, thetaCheck=False)
    fhW_cY_tY = FHDES(pool_classifiers, k=k, theta=theta, mu=NO_Hyperbox_Thereshold, mis_sample_based=False, doContraction=True, thetaCheck=True)
    fhM_cY_tY = FHDES(pool_classifiers, k=k, theta=theta, mu=NO_Hyperbox_Thereshold, mis_sample_based=True,  doContraction=True, thetaCheck=True)
    ...
    list_ds = [majority_voting, single_best, oracle, knorau, kne, desknn, ola, rank, knop, meta,
               fhW_cN_tY, fhM_cN_tY, fhW_cY_tN, fhM_cY_tN, fhW_cY_tY, fhM_cY_tY]
    methods_names = ['MV', 'SB', 'Oracle', 'KNORA-U', 'KNORA-E', 'DESKNN', 'OLA', 'RANK', 'KNOP', 'META-DES',
                     'FHw_cN_tY','FHm_cN_tY','FHw_cY_tN','FHm_cY_tN','FHw_cY_tY','FHm_cY_tY' ]
...
def run_process(datasetName):
    ...
    for itr in range(0, no_itr):
        ...
        state += 1
    write_results_to_file(ExperimentPath, result_one_dataset, predicted_labels, yhat, methods_names, datasetName)
    return result_one_dataset,methods_names,list_ds

theta = .27
NO_Hyperbox_Thereshold = 0.99
ExperimentPath = "ComplexClassifiers_ID3(10)"
NO_classifiers =100
no_itr = 20
